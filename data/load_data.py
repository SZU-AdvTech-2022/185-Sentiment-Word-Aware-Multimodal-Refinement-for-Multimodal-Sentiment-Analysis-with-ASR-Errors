...
class MMDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = {'raw_text': self.rawText[index], 'text': torch.Tensor(self.text[index]),
                  'audio': self.use_audio * torch.Tensor(self.audio[index]),
                  'vision': self.use_video * torch.Tensor(self.vision[index]), 'index': index, 'id': self.ids[index],
                  'labels': {k: torch.Tensor(v[index].reshape(-1)) for k, v in self.labels.items()},
                  'audio_lengths': self.audio_lengths[index], 'vision_lengths': self.vision_lengths[index]}
